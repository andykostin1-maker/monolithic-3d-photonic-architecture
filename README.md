...
* **Electro-optic modulators:** Use of Lithium Niobate ($\text{LiNbO}_3$) electro-optic modulators and phase shifters that modify refractive index ($\Delta n$) via the Pockels effect.
...
* **STAGE 2 (Dynamic reconfigurable matrix):** Dynamic weights are formed by local refractive-index modulation $n(x,y,z)$ using solid-state switches based on PCMs and Lithium Niobate (LiNbO3) phase shifters ($\text{LiNbO}_3$).
...
The chip operates in an isothermal state. Precise temperature control near $25^\circ\text{C}$ requires a micro-Peltier element on the CMOS layer in the 0.5–1 W power range, eliminating the need for bulky liquid-cooling or air-cooling systems.
